...
def build_file_list(args):
    ...
    with open(os.path.join(args.anno_root, 'labels.json')) as f:
        data = json.load(f)
    categories = []
    ...
    files_input = [os.path.join(args.anno_root, 'validation.json'),
                   os.path.join(args.anno_root, 'train.json'),
                   os.path.join(args.anno_root, 'test.json')]
